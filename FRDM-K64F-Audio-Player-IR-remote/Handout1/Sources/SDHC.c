...
void SDHC_sector_read(unsigned char* dataaddress, uint32_t sector);

#define CACHED_SECTORS 3
#define CACHED_NEGATIVE_SECTORS 0 // MUST BE LESS THAN CACHED_SECTORS
#define SECTOR_SIZE 512


typedef struct {
  unsigned char cached_memory[CACHED_SECTORS][SECTOR_SIZE];
	unsigned char *(cache_pointers[CACHED_SECTORS]);
	uint32_t sd_sector_address_of_cache_pointer[CACHED_SECTORS];
	bool sector_need_to_be_read[CACHED_SECTORS];
	int16_t sector_being_read; // 0 to CACHED_SECTORS - 1


} dma_cache_memory;

static dma_cache_memory dma;
...
void SDHC_Init(void){
	...
	// SETUP DMA CACHE STRUCTURE
	dma.sector_being_read = CACHED_SECTORS;
	for (uint16_t pointer_index = 0; pointer_index < CACHED_SECTORS; ++pointer_index){
		dma.cache_pointers[pointer_index] = &dma.cached_memory[pointer_index][0];
		dma.sd_sector_address_of_cache_pointer[pointer_index] = 0;
		dma.sector_need_to_be_read[pointer_index] = false;
	}
...
bool SDHC_dma_update(){
	if((SDHC->IRQSTAT) & SDHC_IRQSTAT_DINT_MASK){ // SD DMA HAS FINISHED
		SDHC->IRQSTAT |= SDHC_IRQSTAT_DINT_MASK; // CLEAR FLAG
		dma.sector_need_to_be_read[dma.sector_being_read] = false;
		++dma.sector_being_read;
		if(dma.sector_being_read < CACHED_SECTORS){
			SDHC_sector_read(dma.cache_pointers[dma.sector_being_read], dma.sd_sector_address_of_cache_pointer[dma.sector_being_read]);
		}
		return true;
	}
	return false;
...
unsigned char* SDHC_get_sector_data(uint32_t sector){
	static unsigned char *cache_pointer_for_swap;
	static uint8_t cache_pointers_index = 0;
	static int32_t wasted_cache_pointers = 0;
	static bool send_sd_dma_read_trigger = false;

	if ((sector >= dma.sd_sector_address_of_cache_pointer[0]) && (sector <= dma.sd_sector_address_of_cache_pointer[CACHED_SECTORS - 1])){
		// FIND CACHE SECTOR, AND THE NUMBER OF CACHE SECTORS THAT HAVE NO MORE USE
		wasted_cache_pointers = (int32_t)(sector) - (int32_t)(dma.sd_sector_address_of_cache_pointer[CACHED_NEGATIVE_SECTORS]);
		if(wasted_cache_pointers < 0){
			return dma.cache_pointers[(int32_t)(sector) - (int32_t)(dma.sd_sector_address_of_cache_pointer[0])];
		}
	}
	else{
		// ALL MEMORY IS LOST
		wasted_cache_pointers = CACHED_SECTORS;
	}

	send_sd_dma_read_trigger = dma.sector_being_read == CACHED_SECTORS;
	dma.sector_being_read -= wasted_cache_pointers;

	// CHECK IF THERE IS A SECTOR BEING READ
	if (dma.sector_being_read < 0){ // IF ITS READING A USELESS SECTOR OF MEMORY
		while (((SDHC->IRQSTAT) & SDHC_IRQSTAT_DINT_MASK) == 0){

		}
		SDHC->IRQSTAT |= SDHC_IRQSTAT_DINT_MASK; // CLEAR FLAG
		dma.sector_being_read = 0; // CORRECT SECTOR TO START POSITION
		send_sd_dma_read_trigger = true;
	}

	if (wasted_cache_pointers != 0){
		// SWAP OLD CACHED SECTOR WITH USEFUL ONES
		for(cache_pointers_index = 0; cache_pointers_index < (CACHED_SECTORS - wasted_cache_pointers); ++cache_pointers_index){
			// SWAP OF POINTERS
			cache_pointer_for_swap = dma.cache_pointers[cache_pointers_index];
			dma.cache_pointers[cache_pointers_index] = dma.cache_pointers[cache_pointers_index + wasted_cache_pointers];
			dma.cache_pointers[cache_pointers_index + wasted_cache_pointers] = cache_pointer_for_swap;
			// UPDATE ADDRESS AND CONDITION
			dma.sd_sector_address_of_cache_pointer[cache_pointers_index] = dma.sd_sector_address_of_cache_pointer[cache_pointers_index + wasted_cache_pointers];
			dma.sector_need_to_be_read[cache_pointers_index] = dma.sector_need_to_be_read[cache_pointers_index + wasted_cache_pointers];
		}

		// SEND TO READ OLD CACHE INDEXES
		for(cache_pointers_index = CACHED_SECTORS - wasted_cache_pointers; cache_pointers_index < CACHED_SECTORS; ++cache_pointers_index){
			if (cache_pointers_index > 0){
				dma.sd_sector_address_of_cache_pointer[cache_pointers_index] = dma.sd_sector_address_of_cache_pointer[cache_pointers_index - 1] + 1;
			}
			else{ // IF ALL CACHE ARE LOST, IT NEED TO READ SECTOR
				dma.sd_sector_address_of_cache_pointer[cache_pointers_index] = sector - CACHED_NEGATIVE_SECTORS;
			}
			dma.sector_need_to_be_read[cache_pointers_index] = true;
		}

		// SEND A TRIGGER TO DMA UPDATE IF NEEDED
		if (send_sd_dma_read_trigger){
				SDHC_sector_read(dma.cache_pointers[CACHED_SECTORS - wasted_cache_pointers], dma.sd_sector_address_of_cache_pointer[CACHED_SECTORS - wasted_cache_pointers]);
		}
	}

	// WAIT TILL FIRST SECTOR IS READ
	while (dma.sector_need_to_be_read[CACHED_NEGATIVE_SECTORS]){
		SDHC_dma_update();
	}

	return dma.cache_pointers[CACHED_NEGATIVE_SECTORS];
}

void SDHC_sector_read(unsigned char* dataaddress, uint32_t sector){
  unsigned long reading_sector = sector * 512;
