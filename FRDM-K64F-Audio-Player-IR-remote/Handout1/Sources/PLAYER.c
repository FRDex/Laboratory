...
#define number_of_songs  17

//	IR BUTTONS	//
typedef enum IR_BUTTONS {	PLAY = 34,	PAUSE = 13,	STOP = 2,	FORWARD = 3,	BACKWARD = 1,	NEXT = 32,	PREVIOUS = 33,	VOLUME_UP = 16,	VOLUME_DOWN = 17	}IR_BUTT;

static uint16_t ISR_index;

static const uint32_t song_list[number_of_songs] = {	110599, 				839,			 476231,	  		15111,			540359,		   584519,				175751,
												// {"i_feel_good_16_bit", "i_feel_good_8_bit", "sex on fire", "the game of love",	"smooth", "sweet child of mine",  "despacito"
														139143,			   		 224263,		59975,		  249863,   288519,		   364231,		 442823,	    403399,
												//"callme when youre soberb", "festa festa", "back in black", "halo", "ignorance", "mr brightside", "roxane", "remmember the name"
														511943,				  326407};
												//"shipping up to Boston", "in the shadows"
typedef struct {
  // SONG VARAIBLES
...
void play_isr(){
  ...
  static int32_t aux_1, aux_2;
  static int32_t min_value_16_bit_resolution = (1 << 15);
  ...
  if (current_song.playing){
    ...
    if(current_song.bit_resolution == 16){
    	channel_1_audio_sample = ((int32_t)(channel_1_audio_sample) + min_value_16_bit_resolution) >> 4;
    	channel_2_audio_sample = channel_1_audio_sample;
    }
...
static bool player_volume_down(){
  ...
  // DAC HAS ONLY 12 BIT RESOLUTION
  if(current_song.volume_reduction > 12){
    current_song.volume_reduction = 12;
  }
  return true;
}
