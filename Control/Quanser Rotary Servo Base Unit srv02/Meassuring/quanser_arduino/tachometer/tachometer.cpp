...
/*SETUP*/
bool tachometer_set(uint8_t pin, bool set_zero_reference, bool radianes, float max_voltage, uint16_t max_resolution, uint16_t sensitivity){
  tachometer_pin = pin;
  voltage = max_voltage;
  resolution = max_resolution;
  tachometer_sensitivity = sensitivity;
  pinMode(tachometer_pin, INPUT);
  tachometer_set_radianes(radianes);
  if(set_zero_reference){
    tachometer_set_actual_value_to_zero_reference();
  }
  tachometer_is_set = true;
  return tachometer_is_set;
}

bool tachometer_set_actual_value_to_zero_reference(){
